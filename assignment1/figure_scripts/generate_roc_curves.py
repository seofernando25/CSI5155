...
def generate_roc_curves_from_results():
    ...
    for sampling_idx, (sampling, sampling_name) in enumerate(
        ...
        for model_idx, (model, model_name) in enumerate(zip(model_keys, model_names)):
            ...
            metrics = find_metrics_for_model(results, model, sampling)
            ...
            if auc_value is not None:
                ...
                fpr = np.linspace(0, 1, 100)
                tpr = fpr
                ax.plot(fpr, tpr, linestyle="--", label="AUC=0.500")
        ...
        add_suptitle(fig, f"ROC Curves - {sampling_name}")
